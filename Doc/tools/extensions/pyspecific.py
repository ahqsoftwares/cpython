...
Body.enum.converters["loweralpha"] = Body.enum.converters[
    "upperalpha"
] = Body.enum.converters["lowerroman"] = Body.enum.converters["upperroman"] = (
    lambda x: None
)
    ...
    def run(self):
        ...
        if info is not new_info:
            if not self._do_args_match(info["args"], new_info["args"]):
                self.logger.warn(
                    "Mismatched arguments for audit-event {}: {!r} != {!r}".format(
                        name, info["args"], new_info["args"]
                    )
                )
    ...
    def run(self):
        ...
        if len(self.arguments) == 3:
            inodes, messages = self.state.inline_text(
                self.arguments[2], self.lineno + 1
            )
...
class PydocTopicsBuilder(Builder):
    ...
    def write(self, *ignored):
        ...
        for label in status_iterator(
            pydoc_topic_labels, "building topics... ", length=len(pydoc_topic_labels)
        ):
