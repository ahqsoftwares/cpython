...
def find_capi_vars(root):
    ...
    for dirname in SOURCE_DIRS:
        for filename in glob.glob(
            os.path.join(glob.escape(os.path.join(ROOT_DIR, dirname)), "**/*.[hc]"),
            recursive=True,
        ):
...
def _is_autogen_var(name):
    return (
        ...
        or name.startswith("rop_id.")
        or
        # Python/graminit.c
        name.startswith("arcs_")
        or name.startswith("states_")
    )
...
def _is_type_var(name):
    ...
    return (
        ...
        or name.endswith(("_doc", "__doc__", "_docstring"))
        or name.endswith("_methods")
        or name.endswith("_fields")
        or name.endswith(("_memberlist", "_members"))
        or name.endswith("_slots")
        or name.endswith(("_getset", "_getsets", "_getsetlist"))
        or name.endswith("_as_mapping")
        or name.endswith("_as_number")
        or name.endswith("_as_sequence")
        or name.endswith("_as_buffer")
        or name.endswith("_as_async")
    )
...
def _is_compiler(name):
    return (
        # Python/Python-ast.c
        name.endswith("_type")
        or name.endswith("_singleton")
        or name.endswith("_attributes")
    )
...
def find_vars(root, globals_filename=GLOBALS_FILE):
    ...
    if nm is None:
        ...
        yield from (
            var
            for var in _find_var_symbols(python, nm, capi_vars, globalnames)
...
def make_sort_key(spec):
    columns = [(col.trim("_"), "_" if col.startswith("_") else "") for col in spec]

    def sort_key(var):
        return tuple(getattr(var, col).lstrip(prefix) for col, prefix in columns)
...
def format_vars(allvars, columns, fmts, widths):
    ...
    for var in allvars:
        ...
        row = fmt.format(*("X" if val is True else val or "" for val in values))
        yield row, 1
    yield div, 0
...
COLUMN_FORMATS = {col: "{:%s}" % width for col, width in COLUMN_WIDTHS.items()}
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--filters", default="-scope", help="[[-]<COLUMN>[=<GLOB>]] ..."
    )

    parser.add_argument(
        "--columns", default=COLUMNS, help="a comma-separated list of columns to show"
    )
    parser.add_argument(
        "--sort",
        default="filename,_name",
        help="a comma-separated list of columns to sort",
    )
    parser.add_argument(
        "--group", help="group by the given column name (- to not group)"
    )
...
def main(
    ...
    allvars = (var for var in find_vars(root, filename) if filter_var(var, filters))
    if sort:
