...
class TexinfoParser:
    ...
    FN_ID_PATTERN = "(%(id)s)"
    FN_SOURCE_PATTERN = (
        '<A NAME=footnoteref%(id)s HREF="#footnotetext%(id)s">' + FN_ID_PATTERN + "</A>"
    )
    FN_TARGET_PATTERN = (
        "<A NAME=footnotetext%(id)s"
        ' HREF="#footnoteref%(id)s">' + FN_ID_PATTERN + "</A>\n%(text)s<P>\n"
    )
    FN_HEADER = (
    ...
    def parserest(self, fp, initial_lineno):
        ...
        while not self.done:
            ...
            if mo:
                ...
                blprog.match(line)
                and "format" not in self.stack
                and "example" not in self.stack
            ):
                if accu:
                    if not self.skip:
                        self.process(accu)
    ...
    def endnode(self):
        ...
        if self.nodefp:
            ...
            if not self.cont and (
                not self.node.type
                or (self.node.next and self.node.prev and self.node.up)
            ):
                self.node.finalize()
                self.node.flush()
            else:
    ...
    def open_ss(self):
        self.write("&#223;")  # es-zet or sharp S

    def close_ss(self):
        pass

    def open_oe(self):
        self.write("oe")  # oe ligatures
    ...
    def close_inforef(self):
        ...
        args = [s.trim() for s in text.split(",")]
        while len(args) < 3:
            args.append("")
        node = args[0]
        file = args[2]
    ...
    def command(self, line, mo):
        ...
        if self.debugging > 1:
            print(
                "!" * self.debugging, "command:", self.skip, self.stack, "@" + cmd, args
            )
        try:
    ...
    def do_node(self, args):
        self.endnode()
        self.nodelineno = 0
        parts = [s.trim() for s in args.split(",")]
        while len(parts) < 4:
            parts.append("")
        ...
        self.node = self.Node(
            self.dirname, self.nodename, self.topname, title, next, prev, up
        )
    ...
    def do_item(self, args):
        ...
        if self.itemarg:
            if (
                self.itemarg[0] == "@"
                and self.itemarg[1]
                and self.itemarg[1] in string.ascii_letters
            ):
    ...
    def prindex(self, name):
        ...
        if self.debugging:
            print(
                "!" * self.debugging, "--- Generating", self.indextitle[name], "index"
            )
...
class TexinfoParserHTML3(TexinfoParser):
    ...
    FN_ID_PATTERN = "[%(id)s]"
    FN_SOURCE_PATTERN = (
        '<A ID=footnoteref%(id)s HREF="#footnotetext%(id)s">' + FN_ID_PATTERN + "</A>"
    )
    FN_TARGET_PATTERN = (
...
class HTMLHelp:
    ...
    def dumpnode(self, node, indent=0, outfile=sys.stdout):
        if node:
            ...
            print(
                '<param name="Local" value="' + filename + '">', end=" ", file=outfile
            )
    ...
    def dumpindex(self, outfile=sys.stdout):
        ...
        for key, location in self.indexlist:
            ...
            print(
                '<param name="Local" value="' + location + '">', end=" ", file=outfile
            )
            print("</OBJECT>", file=outfile)
        print("</UL>", file=outfile)
...
def findwordend(str, i, n):
    ...
    while i < n:
        ...
        i = i + 1
        if c == "@":
            i = i + 1  # Next character is not special
        elif c == "{":
            level = level + 1
        elif c == "}":
            level = level - 1
...
def test():
    ...
    if len(sys.argv) != 3:
        print(
            "usage: texi2hh [-d [-d]] [-p] [-c] [-3] [-H htmlhelp]",
