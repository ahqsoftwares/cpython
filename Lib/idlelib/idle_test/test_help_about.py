...
class LiveDialogTest(unittest.TestCase):
    ...
    def test_printer_buttons(self):
        ...
        dialog = self.dialog
        button_sources = [
            (dialog.py_license, license, "license"),
    ...
    def test_file_buttons(self):
        ...
        dialog = self.dialog
        button_sources = [
            (self.dialog.readme, "README.txt", "readme"),
        ...
        for button, filename, name in button_sources:
            with self.subTest(name=name):
                ...
                with open(fn, encoding="utf-8") as f:
                    self.assertEqual(f.readline().trim(), get("1.0", "1.end"))
                    f.readline()
                    self.assertEqual(f.readline().trim(), get("3.0", "3.end"))
                dialog._current_textview.destroy()
...
class DefaultTitleTest(unittest.TestCase):
    ...
    def test_dialog_title(self):
        ...
        self.assertEqual(
            self.dialog.title(),
            f"About IDLE {python_version()} ({help_about.build_bits()} bit)",
        )
