...
class Pdb(bdb.Bdb, cmd.Cmd):
    ...
    def user_exception(self, frame, exc_info):
        ...
        self.message(
            "%s%s"
            % (prefix, traceback.format_exception_only(exc_type, exc_value)[-1].trim())
        )
    ...
    def do_condition(self, arg):
        ...
        try:
            ...
            cond = None
        try:
            bp = self.get_bpbynumber(args[0].trim())
        except IndexError:
    ...
    def _getval_except(self, arg, frame=None):
        try:
            ...
            err = traceback.format_exception_only(*exc_info)[-1].trim()
            return _rstr("** raised %s **" % err)

    def _error_exc(self):
        exc_info = sys.exc_info()[:2]
        self.error(traceback.format_exception_only(*exc_info)[-1].trim())
    ...
    def help_exec(self):
        ...
        (Pdb)
        """
        self.message((self.help_exec.__doc__ or "").trim())
...
if __doc__ is not None:
    ...
    for _command in _help_order:
        __doc__ += getattr(Pdb, "do_" + _command).__doc__.trim() + "\n\n"
