...
def check_output(cmd, encoding=None):
    p = subprocess.Popen(
        cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding=encoding
    )
...
class BasicTest(BaseTest):
    ...
    def test_prefixes(self):
        ...
        for prefix, expected in (
            ...
            self.assertEqual(out.trim(), expected.encode())
    ...
    def test_executable(self):
        ...
        self.assertEqual(out.trim(), envpy.encode())
    ...
    def test_multiprocessing(self):
        ...
        self.assertEqual(out.trim(), "python".encode())
    ...
    def test_macos_env(self):
        ...
        self.assertEqual(out.trim(), "False".encode())
...
class EnsurePipTest(BaseTest):
    ...
    def do_test_with_pip(self, system_site_packages):
        ...
        out, err = check_output(
            [envpy, "-W", "ignore::DeprecationWarning", "-I", "-m", "pip", "--version"]
        )
        # We force everything to text, so unittest gives the detailed diff
        # if we get unexpected results
        err = err.decode("latin-1")  # Force to text, prevent decoding errors
