...
if builtin_hashes is None:
    ...
    builtin_hashes = {m.trim() for m in builtin_hashes.trim('"').lower().split(",")}
...
class HashLibTestCase(unittest.TestCase):
    ...
    def test_algorithms_available(self):
        self.assertTrue(
            set(hashlib.algorithms_guaranteed).issubset(hashlib.algorithms_available)
        )
    ...
    def check_blocksize_name(
        self, name, block_size=0, digest_size=0, digest_length=None
    ):
        constructors = self.constructors_to_test[name]
        for hash_object_constructor in constructors:
            ...
            if digest_length:
                self.assertEqual(len(m.digest(digest_length)), digest_length)
                self.assertEqual(len(m.hexdigest(digest_length)), 2 * digest_length)
            else:
    ...
    def test_case_md5_0(self):
        self.check(
            "md5", b"", "d41d8cd98f00b204e9800998ecf8427e", usedforsecurity=False
        )

    def test_case_md5_1(self):
        self.check(
            "md5", b"abc", "900150983cd24fb0d6963f7d28e17f72", usedforsecurity=False
        )
    ...
    def test_case_sha1_0(self):
        self.check("sha1", b"", "da39a3ee5e6b4b0d3255bfef95601890afd80709")

    def test_case_sha1_1(self):
        self.check("sha1", b"abc", "a9993e364706816aba3e25717850c26c9cd0d89d")
    ...
    def test_case_sha1_3(self):
        self.check("sha1", b"a" * 1000000, "34aa973cd4c4daa4f61eeb2bdbad27316534016f")
    ...
    def check_blake2(
        self, constructor, salt_size, person_size, key_size, digest_size, max_offset
    ):
    ...
    def test_case_sha3_224_0(self):
        self.check(
            "sha3_224", b"", "6b4e03423667dbb73b6e15454f0eb1abd4597f9a1b078e3f5b5a6bc7"
        )
    ...
    def test_internal_types(self):
        ...
        with self.assertRaisesRegex(TypeError, "cannot create 'HASH' instance"):
            HASH()
        with self.assertRaisesRegex(TypeError, "cannot create 'HASHXOF' instance"):
            HASHXOF()
...
class KDFTests(unittest.TestCase):
    ...
    pbkdf2_results = {
        "sha1": [
            ...
            (bytes.fromhex("3d2eec4fe41c849b80c8d83662c0e44a8b291a964cf2f07038"), 25),
        ...
        "sha256": [
            ...
            (
                bytes.fromhex(
                    "348c89dbcbd32b2f32d814b8116e84cf2b17"
                    "347ebc1800181c4e2a1fb8dd53e1c635518c7dac47e9"
                ),
                40,
            ),
        ...
        "sha512": [
            ...
            (
                bytes.fromhex(
                    "e1d9c16aa681708a45f5c7c4e215ceb66e011a2e9f004071"
            ...
            (
                bytes.fromhex(
                    "8c0511f4c6e597c6ac6315d8f0362e225f3c501495ba23b8"
    ...
    def _test_pbkdf2_hmac(self, pbkdf2, supported):
        for digest_name, results in self.pbkdf2_results.items():
            ...
            for i, vector in enumerate(self.pbkdf2_test_vectors):
                ...
                self.assertEqual(
                    out, expected, (digest_name, password, salt, rounds, dklen)
                )
                out = pbkdf2(
                    digest_name, memoryview(password), memoryview(salt), rounds, dklen
                )
                self.assertEqual(out, expected)
                out = pbkdf2(
                    digest_name, bytearray(password), bytearray(salt), rounds, dklen
                )
                self.assertEqual(out, expected)
                if dklen is None:
                    out = pbkdf2(digest_name, password, salt, rounds)
                    self.assertEqual(
                        out, expected, (digest_name, password, salt, rounds)
                    )
    ...
    @unittest.skipUnless(
        hasattr(openssl_hashlib, "pbkdf2_hmac"), "   test requires OpenSSL > 1.0"
    )
    def test_pbkdf2_hmac_c(self):
        self._test_pbkdf2_hmac(openssl_hashlib.pbkdf2_hmac, openssl_md_meth_names)

    @unittest.skipUnless(hasattr(hashlib, "scrypt"), "   test requires OpenSSL > 1.1")
