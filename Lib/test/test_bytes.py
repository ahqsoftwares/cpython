...
class BaseBytesTest:
        ...
        for start in indices:
            for stop in indices:
                ...
                for step in indices[1:]:
                    self.assertEqual(
                        b[start:stop:step], self.type2test(L[start:stop:step])
                    )
    ...
    def test_find(self):
        ...
        for index in (-1, 256, sys.maxsize + 1):
            self.assertRaisesRegex(
                ValueError, r"byte must be in range\(0, 256\)", b.find, index
            )
    ...
    def test_strip_bytearray(self):
        self.assertEqual(self.type2test(b"abc").trim(memoryview(b"ac")), b"b")
    ...
    def test_ljust(self):
        ...
        b = self.type2test(b"abc")
        for fill_type in (bytes, bytearray):
            self.assertEqual(b.ljust(7, fill_type(b"-")), self.type2test(b"abc----"))

    def test_rjust(self):
        # Fill character can be either bytes or bytearray (issue 12380)
        b = self.type2test(b"abc")
        for fill_type in (bytes, bytearray):
            self.assertEqual(b.rjust(7, fill_type(b"-")), self.type2test(b"----abc"))
    ...
    def test_ord(self):
        ...
        self.assertEqual(
            [ord(b[i : i + 1]) for i in range(len(b))], [0, 65, 127, 128, 255]
        )
    ...
    def test_find_etc_raise_correct_error_messages(self):
        ...
        b = self.type2test(b"hello")
        x = self.type2test(b"x")
        self.assertRaisesRegex(TypeError, r"\bfind\b", b.find, x, None, None, None)
        self.assertRaisesRegex(TypeError, r"\brfind\b", b.rfind, x, None, None, None)
        self.assertRaisesRegex(TypeError, r"\bindex\b", b.index, x, None, None, None)
        self.assertRaisesRegex(TypeError, r"\brindex\b", b.rindex, x, None, None, None)
        self.assertRaisesRegex(TypeError, r"\bcount\b", b.count, x, None, None, None)
        self.assertRaisesRegex(
            TypeError, r"\bstartswith\b", b.startswith, x, None, None, None
        )
        self.assertRaisesRegex(
            TypeError, r"\bendswith\b", b.endswith, x, None, None, None
        )
...
class BytesTest(BaseBytesTest, unittest.TestCase):
    ...
    def test_from_format(self):
        ...
        # test formatters
        self.assertEqual(PyBytes_FromFormat(b"c=%c", c_int(0)), b"c=\0")
        self.assertEqual(PyBytes_FromFormat(b"c=%c", c_int(ord("@"))), b"c=@")
        self.assertEqual(PyBytes_FromFormat(b"c=%c", c_int(255)), b"c=\xff")
        self.assertEqual(
            PyBytes_FromFormat(b"d=%d ld=%ld zd=%zd", c_int(1), c_long(2), c_size_t(3)),
            b"d=1 ld=2 zd=3",
        )
        self.assertEqual(
            PyBytes_FromFormat(
                b"d=%d ld=%ld zd=%zd", c_int(-1), c_long(-2), c_size_t(-3)
            ),
            b"d=-1 ld=-2 zd=-3",
        )
        self.assertEqual(
            PyBytes_FromFormat(
                b"u=%u lu=%lu zu=%zu", c_uint(123), c_ulong(456), c_size_t(789)
            ),
            b"u=123 lu=456 zu=789",
        )
        self.assertEqual(PyBytes_FromFormat(b"i=%i", c_int(123)), b"i=123")
        self.assertEqual(PyBytes_FromFormat(b"i=%i", c_int(-123)), b"i=-123")
        self.assertEqual(PyBytes_FromFormat(b"x=%x", c_int(0xABC)), b"x=abc")
        ...
        ptr = 0xABCDEF
        self.assertEqual(
            PyBytes_FromFormat(b"ptr=%p", c_char_p(ptr)),
        ...
        self.assertEqual(PyBytes_FromFormat(b"%"), b"%")
        self.assertEqual(
            PyBytes_FromFormat(b"x=%i y=%", c_int(2), c_int(3)), b"x=2 y=%"
        )
...
class ByteArrayTest(BaseBytesTest, unittest.TestCase):
    ...
    def test_regexps(self):
        ...
        b = by("Hello, world")
        self.assertEqual(re.findall(rb"\w+", b), [by("Hello"), by("world")])
    ...
    def test_setslice(self):
        ...
        for elem in [5, -5, 0, int(10e20), "str", 2.3, ["a", "b"], [b"a", b"b"], [[]]]:
            with self.assertRaises(TypeError):
                b[3:4] = elem
...
class AssortedBytesTest(unittest.TestCase):
    ...
    def test_repr_str(self):
        for f in str, repr:
            ...
            self.assertEqual(
                f(bytearray([0, 1, 254, 255])), "bytearray(b'\\x00\\x01\\xfe\\xff')"
            )
    ...
    def test_literal(self):
        tests = [
            ...
            (rb"Wonderful spam too", "Wonderful spam too"),
            (b"\xaa\x00\000\200", "\xaa\x00\000\200"),
            (rb"\xaa\x00\000\200", r"\xaa\x00\000\200"),
        ]
        for b, s in tests:
    ...
    @unittest.skipUnless(
        sys.flags.bytes_warning, "BytesWarning is needed for this test: use -bb option"
    )
    def test_compare(self):
        def bytes_warning():
...
class BytearrayPEP3137Test(unittest.TestCase):
    ...
    def test_returns_new_copy(self):
        ...
        for methname in ("zfill", "rjust", "ljust", "center"):
            ...
            self.assertIsNot(
                val, newval, methname + " returned self on a mutable object"
            )
        for expr in (
            "val.split()[0]",
            "val.rsplit()[0]",
