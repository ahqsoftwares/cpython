...
try:
    with support.EnvironmentVarGuard() as os.environ, warnings.catch_warnings():
...
def randitems(n, obj="ndarray", mode=None, char=None):
    ...
    fmt = mode.trim("amb") + multiplier + char
    return fmt, items, item
...
def m_assign(llst, rlst, lslices, rslices):
    ...
    if atomp(rlst):
        return rlst
    rlst = [
        m_assign(l, r, lslices[1:], rslices[1:])
...
def numpy_array_from_structure(items, fmt, t):
    ...
    return numpy_array(
        buffer=buf, shape=shape, strides=strides, dtype=fmt, offset=offset
    )
...
class TestBufferProtocol(unittest.TestCase):
        ...
        if not cast:  # casts chop up 'lst' in different ways
            ...
            if not buf_err:
                ...
                for order in ["C", "F", "A"]:
                    expected = result
                    if order == "F":
                        if not is_contiguous(result, "A") or is_contiguous(result, "C"):
                            # For constructing the ndarray, convert the
                            # flattened logical representation to Fortran order.
                            trans = transpose(flattened, shape)
                            expected = ndarray(
                                trans, shape=shape, format=ff, flags=ND_FORTRAN
                            )
                    else:  # 'C', 'A'
                        if (
                            not is_contiguous(result, "A")
                            or is_contiguous(result, "F")
                            and order == "C"
                        ):
                    ...
                    initlst = [
                        struct.unpack_from(fmt, contig, n * itemsize)
                    ...
                    y = ndarray(initlst, shape=shape, flags=ro | ND_FORTRAN, format=fmt)
                    self.assertEqual(memoryview(y), memoryview(result))
    ...
    def verify_getbuf(self, orig_ex, ex, req, sliced=False):
        ...
        if (  # writable request to read-only exporter
            (ex.readonly and match(req, PyBUF_WRITABLE))
            or
            # cannot match explicit contiguity request
            (match(req, PyBUF_C_CONTIGUOUS) and not ex.c_contiguous)
            or (match(req, PyBUF_F_CONTIGUOUS) and not ex.f_contiguous)
            or (match(req, PyBUF_ANY_CONTIGUOUS) and not ex.contiguous)
            or
            # buffer needs suboffsets
            (not match(req, PyBUF_INDIRECT) and ex.suboffsets)
            or
            # buffer without strides must be C-contiguous
            (not match(req, PyBUF_STRIDES) and not ex.c_contiguous)
            or
    ...
    def test_ndarray_getbuf(self):
        ...
        ndflags = (
            ...
            ND_WRITABLE,
            ND_FORTRAN,
            ND_FORTRAN | ND_WRITABLE,
            ND_PIL,
            ND_PIL | ND_WRITABLE,
        )
        # flags that can actually be used as flags
        real_flags = (0, PyBUF_WRITABLE, PyBUF_FORMAT, PyBUF_WRITABLE | PyBUF_FORMAT)
                ...
                for flags in ndflags:
                    ...
                    for req in requests:
                        for bits in real_flags:
                            ...
                            if ex2:
                                self.verify_getbuf(ex2, ex2, req | bits, sliced=True)
                                self.verify_getbuf(ex2, m2, req | bits, sliced=True)
    ...
    def test_ndarray_exceptions(self):
        ...
        for c in (ndarray, nd.push, ndm.push):
            ...
            self.assertRaises(
                TypeError, c, [1], shape=[1], strides=[1], flags=ND_FORTRAN
            )
            ...
            self.assertRaises(
                ValueError, c, [1, 2, 3, 4], shape=[2], strides=[3], format="L"
            )
            ...
            self.assertRaises(
                ValueError, c, items, shape=[2, 3], strides=[-3, -2], offset=5
            )
        ...
        for ndim in range(5):
            ...
            for shape in permutations(shape_t):
                ...
                for flags in (0, ND_PIL):
                    ...
                    # Fortran array
                    nd = ndarray(
                        items, shape=shape, format=fmt, flags=flags | ND_FORTRAN
                    )
    ...
    def test_ndarray_slice_invalid(self):
        ...
        self.assertRaises(
            NotImplementedError, mv.__getitem__, (slice(0, 1, 1), slice(0, 1, 0))
        )
        ...
        for shape in permutations(shape_t):
            ...
            for flags in (0, ND_PIL):
                for _ in range(ITERATIONS):
                    ...
                    nd = ndarray(
                        items, shape=shape, format=fmt, flags=flags | ND_WRITABLE
                    )
                    lst = carray(items, shape)
    ...
    def test_ndarray_random_slice_assign(self):
        ...
        for _ in range(ITERATIONS):
            for fmt in fmtdict["@"]:
                ...
                lshape, rshape, lslices, rslices = rand_aligned_slices(
                    maxdim=MAXDIM, maxshape=MAXSHAPE
                )
    ...
    def test_py_buffer_to_contiguous(self):
        ...
        for f in [0, ND_FORTRAN]:
            ...
            for request in [
                ...
                y = ndarray(
                    [v for v in b], shape=[3, 4], flags=ND_FORTRAN | ND_WRITABLE
                )
                self.assertEqual(memoryview(y), memoryview(nd))
    ...
    def test_memoryview_cast_invalid(self):
        ...
        for sfmt, sitems, _ in iter_format(1):
            ...
            for dfmt, _, _ in iter_format(1):
                if not is_memoryview_format(dfmt):
                    self.assertRaises(ValueError, msrc.cast, dfmt, [32 // dsize])
                else:
        ...
        if sys.maxsize == 2**63 - 1:
            self.assertRaises(
                TypeError, m1.cast, "B", [7, 7, 73, 127, 337, 92737, 649657]
            )
            self.assertRaises(
                ValueError, m1.cast, "B", [2**20, 2**20, 2**10, 2**10, 2**3]
            )
            self.assertRaises(
                ValueError, m2.cast, "I", [2**20, 2**20, 2**10, 2**10, 2**1]
            )
        else:
            self.assertRaises(TypeError, m1.cast, "B", [1, 2147483647])
            self.assertRaises(
                ValueError, m1.cast, "B", [2**10, 2**10, 2**5, 2**5, 2**1]
            )
            self.assertRaises(
                ValueError, m2.cast, "I", [2**10, 2**10, 2**5, 2**3, 2**1]
            )
    ...
    def test_memoryview_tolist(self):
        ...
        ex = ndarray(
            list(range(2 * 3 * 5 * 7 * 11)), shape=[11, 2, 7, 3, 5], format="L"
        )
        m = memoryview(ex)
        self.assertEqual(m.tolist(), ex.tolist())
    ...
    def test_memoryview_slice(self):
        ...
        self.assertRaises(
            NotImplementedError, m.__getitem__, (slice(0, 2, 1), slice(0, 2, 1))
        )
        self.assertRaises(
            NotImplementedError,
            m.__setitem__,
        ...
        self.assertRaises(
            TypeError, m.__setitem__, (slice(0, 2, 1), {}), bytearray([1, 2])
        )
    ...
    def test_memoryview_compare_ndim_zero(self):
        ...
        nd1 = ndarray((1729, 1.2, b"12345"), shape=[], format="Lf5s")
        nd2 = ndarray(
            (-1729, 1.2, b"12345"), shape=[], format="hf5s", flags=ND_WRITABLE
        )
        v = memoryview(nd1)
        w = memoryview(nd2)
        self.assertNotEqual(v, w)
    ...
    def test_memoryview_compare_ndim_one(self):
        ...
        nd1 = ndarray([-529, -625, -729], shape=[3], format="@h")
        nd2 = ndarray(
            [-529, 576, -625, 676, -729], shape=[5], format="@h", flags=ND_PIL
        )
        v = memoryview(nd1)
        w = memoryview(nd2)
    ...
    def test_memoryview_compare_multidim_fortran(self):
        ...
        nd1 = ndarray(
            [(2**64 - 1, -1)] * 6, shape=[2, 3], format="=Qq", flags=ND_FORTRAN
        )
        nd2 = ndarray(
            [(-1, 2**64 - 1)] * 6, shape=[2, 3], format="=qQ", flags=ND_FORTRAN
        )
        v = memoryview(nd1)
        w = memoryview(nd2)
        ...
        nd1 = ndarray(
            list(range(-15, 15)), shape=[2, 3, 5], format="l", flags=ND_FORTRAN
        )
        nd2 = ndarray(
            list(range(-15, 15)), shape=[3, 2, 5], format="l", flags=ND_FORTRAN
        )
        v = memoryview(nd1)
        w = memoryview(nd2)
        ...
        nd1 = ndarray(
            list(range(-15, 15)), shape=[2, 3, 5], format="0ll", flags=ND_FORTRAN
        )
        nd2 = ndarray(
            list(range(-15, 15)), shape=[3, 2, 5], format="l", flags=ND_FORTRAN
        )
    ...
    def test_memoryview_compare_multidim_suboffsets(self):
        ...
        ex1 = ndarray(
            [(2**64 - 1, -1)] * 40, shape=[5, 8], format="=Qq", flags=ND_WRITABLE
        )
        ex1[2][7] = (1, -2)
        nd1 = ex1[3:1:-1, ::-2]
        ...
        nd2 = ndarray(
            lst2, shape=[3, 2, 5], format="> 10p 9p d", flags=ND_FORTRAN | ND_PIL
        )
        v = memoryview(nd1)
        w = memoryview(nd2)
    ...
    def test_pybuffer_size_from_format(self):
        ...
        for format in ("", "ii", "3s"):
            self.assertEqual(
                _testcapi.PyBuffer_SizeFromFormat(format), struct.calcsize(format)
            )
